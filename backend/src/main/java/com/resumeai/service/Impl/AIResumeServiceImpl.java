...
public class AIResumeServiceImpl extends ResumeServiceImpl implements AIResumeService {
    ...
    public Resume sendResume() {
        ...
        if(enhancedResumeCache.containsKey(user.getId())) {
            resume = enhancedResumeCache.get(user.getId());
        } else {
            resume = enhanceResume();
            enhancedResumeCache.put(user.getId(), resume);
        }
    ...
    private Resume enhanceResume() {
        ...
        resume.setSummary(cleanSummaryHtml(refineSection("summary", user.getSummary())));
        resume.setSkills(sanitizeAndValidateHtml(refineSection("skills", resumeData.getSkills())));
        resume.setProjects(sanitizeAndValidateHtml(refineSection("projects", resumeData.getProjects())));
        resume.setEducation(sanitizeAndValidateHtml(refineSection("education", resumeData.getEducations())));
        if (!user.isFresher()) {
            resume.setWorkExperience(sanitizeAndValidateHtml(refineSection("work experience", resumeData.getWorkExperiences())));
        }
    ...
    public String cleanSummaryHtml(String html) {
        ...
        try {
            ...
            // Remove unwanted symbols, keeping letters, numbers, and spaces
            // Adjust the regex as needed. This keeps a-z, A-Z, 0-9 and spaces.
            String cleanedText = text.replaceAll("[^a-zA-Z0-9\\s]", "").replaceAll("\\s{2,}", " ").trim();

            // Wrap the cleaned text back in a <p> tag
            return "<p>" + cleanedText + "</p>";
        } catch (Exception e) {
            System.out.println("Error cleaning summary HTML: " + e.getMessage());
            return html;
        }
    }
    ...
    private String formatEducation(Object rawContent) {
        ...
        for (EducationDTO edu : educations) {
            ...
            builder.append("End Date: ")
                    .append(formatDate(edu.getEndDate(), edu.isCurrentlyStudying()))
                    .append("\n");
    ...
    // this will be send to thymeleaf
    private String renderResume(Resume resume) {
        User user = getLoggedInUser();
        Context context = new Context();

        if(enhancedResumeCache.get(user.getId()) == null) {
            context.setVariable("resume", resume);
        } else {
            context.setVariable("resume", enhancedResumeCache.get(user.getId()));
        }
    ...
    public String saveResume(Resume resume) {
        User user = getLoggedInUser();
        // Save or update the resume in the cache or database
        try {
            // throws error if html is compatible
            renderResume(resume);
        } catch (Exception e) {
            log.error("Error rendering resume for user {}: {}", user.getId(), e.getMessage());
        ...
        savedResume.setName(resume.getName());
        savedResume.setEmail(resume.getEmail());
        savedResume.setSummary(resume.getSummary());
        savedResume.setSkills(resume.getSkills());
        savedResume.setProjects(resume.getProjects());
        savedResume.setEducation(resume.getEducation());
        savedResume.setWorkExperience(resume.getWorkExperience());
